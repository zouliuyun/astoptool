...
rm -f ${server_lys}.tgz
###########
sed -i "s/\(.*port.*>\).*\(<.*\)/\1${select_port}\2/g" /app/${servername}/backend/conf.xml
sed -i "s/\(.*httpPort.*>\).*\(<.*\)/\1$http_port\2/g" /app/${servername}/backend/conf.xml
#下载公共的包{font,lib等等}
...
clear_db=1
if [ $DATE2 -lt $DATE1 -a $deploy_type == "recover" ];then
    DATE_DB=`date +%Y%m%d`
    ...
    do
        ...
        rm -f $sqlfilename
        curl --retry 3 --retry-delay 10 --retry-max-time 30 -X GET --header "Content-Type: application/octet-stream" "http://122.225.114.68:14000/webhdfs/v1/$FTP_PATH/$servername/$sqlfilename?op=open&user.name=hdfs&data=true" -o $sqlfilename.bz2

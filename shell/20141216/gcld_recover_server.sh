...
#DATE_DB=`date -d "1 days ago" +%Y%m%d`
DATE_DB=`date +%Y%m%d`
db_file=${servername}_${DATE_DB}.sql.bz2
db_name=${servername}_${DATE_DB}.sql
...
sed -i "s/gcld.serverchinesename = .*/gcld.serverchinesename = $server_cname/g" server.properties
#联运配置文件修改
sed -i "s/\(gcld.game.url =\s*https\?:\/\/\)[^\/]*\(.*\)/\1${dns_name}\2/g" /app/${servername}/backend/apps/${server_lys}.properties
sed -i "s/\(gcld.servername = \).*/\1${server_lys}/g" /app/${servername}/backend/apps/${server_lys}.properties
sed -i "s/\(gcld.serverid = \).*/\1$server_id/g" /app/${servername}/backend/apps/${server_lys}.properties
sed -i "s/\(gcld.serverids = \).*/\1S$server_id/g" /app/${servername}/backend/apps/${server_lys}.properties
#修改http端口
sed -i "s/\(.*httpPort.*>\).*\(<.*\)/\1$http_port\2/g" /app/${servername}/backend/apps/conf.xml
#修改数据库名称
sed -i "s/\(.*jdbc.*127.0.0.1:3306\/${game}_\).*[0-9]<\(.*\)/\1${server_flag}<\2/g" /app/${servername}/backend/apps/applicationContext.xml
sed -i "s/\(.*127.0.0.1:3306\/${game}_\).*?\(.*\)/\1${server_flag}?\2/g" /app/${servername}/backend/apps/applicationContext.xml
#修改运营商标示
sed -i "s/\(gcld.yx = \).*/\1${server_lys}/g" /app/${servername}/backend/apps/server.properties
#修改战报路径
sed -i "s/\(gcld.report.linux.path.*app\).*[0-9]\(.*\)/\1\/${servername}\2/g" /app/${servername}/backend/apps/server.properties
#修改跨服战唯一标示key
sed -i "s/\(gcld.serverkey = \).*/\1${server_lys}_S${server_id}/g" /app/${servername}/backend/apps/server.properties
#修改后端端口
...
sed -i "/gcld.server.time=/d"  /app/${servername}/backend/apps/serverstate2.properties
...
if [ $clear_db -eq 1 ];then
    ...
    do
        ...
        db_name=${servername}_${DATE_DB}.sql
        if [ ! -f $db_file ];then
            curl --retry 3 --retry-delay 10 --retry-max-time 30 -X GET --header "Content-Type: application/octet-stream" "http://122.225.114.68:14000/webhdfs/v1/$FTP_PATH/$servername/$db_file?op=open&user.name=hdfs&data=true" -o $db_file
        fi
        bzip2 -d -k $db_file
        if [ $? -eq 0 ];then
            echo "decompress $db_file ok"
            break
        else
            if [ $i -gt 4 ];then
                DATE_DB=`date -d "$BASE_DATE -1 days" +%Y%m%d`
            fi
            echo "decompress $db_file fail"
            rm -f $db_file $db_name
        fi
    done
    if [ -f $db_name ];then
        pandora --update -e "create database ${servername}"
        pandora --update ${servername} < $db_name
        rm -f $db_file $db_name
    else
        echo "download $db_file fail"
        exit 1
    fi
fi
#重新加载nginx配置
...
if [ $clear_db -eq 1 ];then
    ...
    start_time=`pandora $servername -e "select server_time from db_version"|grep -v 'server_time'`
    server_time=`date -d "$start_time" +%s`
    sed -i "/gcld.server.time/d" /app/${servername}/backend/apps/serverstate.properties
    echo "gcld.server.time=${server_time}000" >> /app/${servername}/backend/apps/serverstate.properties
    echo "$server_flag不需要清档或者清档时间已过！"
fi
#启动游戏

...
if [ "$server_lys" == "renren" ];then
    ...
    sed -i "s/s[0-9]*.zzsf2.renren.com/$dns_name/g" $www_dir/index.html
    sed -i "s/x[0-9]*.zzsf2.renren.com/$dnsname_renren/g" $www_dir/Main.html
    sed -i "s/s[0-9]*.zzsf2.renren.com/$dns_name/g" $backend_dir/${server_lys}.properties
...
sed -i "s/Main.html/$Main/g" /app/nginx/conf/vhost/${game}_${server_flag}.conf

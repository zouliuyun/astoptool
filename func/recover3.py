...
import check,common,ssh,mailhtml,sendmail,serverlist,getip

class recover:
    def __init__(self,game,language,servername,ip,recoverType):
        '''游戏恢复任务'''
        self.game = game
    ...
    def init(self):
        ...
        try:
            ...
            self.dnsSsh = ssh.ssh("10.6.196.65")
            logging.info("10.6.196.65域名解析api机器建立ssh连接完毕!")
            ...
            #主服部署通用脚本
            self.deploy_common_script = "deploy_common.sh"
            #混服部署脚本
            self.mix_deploy_script = gameOption("mix_deploy_script")
            #部署混服的通用脚本
            self.deployCommonScript = "deploy_mix_common.sh"
            #执行特殊脚本
    ...
    def getUrl(self,yx,quhao):
        ...
        try:
            ...
            if urlinfo.strip() == "":
                self.statusCheck(False,"联运：%s asturl或者gameurl在模板中不存在" %yx)
            else:
                ...
                if len(urlinfoline) == 5:
                    ...
                    if gameurl.find("${serverid}") >= 0 :
                        server_url = gameurl.replace("${serverid}",quhao)
                    if asturl.find("${serverid}") >= 0 :
                        asturl = asturl.replace("${serverid}",quhao)
                    if title.find("${serverid}") >= 0:
                        title = title.replace("${serverid}",quhao)
            if self.add_dns :
                if asturl == None or asturl.strip() == "":
                    self.statusCheck(False,"获取ast域名失败")
                else:
                    server_url_tag = asturl.replace(".aoshitang.com","")
                    ast_full_url = server_url_tag + ".aoshitang.com"
            #游戏url
            if server_url == None or server_url.strip() == "":
                self.statusCheck(False,"获取游戏域名失败")
            if title == None or title.strip() == "":
                title = "%s %s"%(yx,quhao)
            web_url = self.web_url.replace("${server_url}",server_url)
            return {"web_url":web_url,"title":title,"server_url_tag":server_url_tag,"server_url":server_url,"asturl":ast_full_url}
        except Exception,e1:
            self.statusCheck(False,str(e1))
...
#        logging.info("邮件已发送")
    def dnsJiexi(self,server_url_tag):
        try:
            if self.add_dns:
                #dnsSsh = ssh.ssh("10.6.196.65")
                status,stdout,stderr = self.dnsSsh.cmd("/app/opbin/dns/dnsapi -g %s -a up -d %s -l 1 -i %s"%(self.dns_game,server_url_tag,self.dianxinIp))
                logging.info("/app/opbin/dns/dnsapi -g %s -a up -d %s -l 1 -i %s"%(self.dns_game,server_url_tag,self.dianxinIp))
                logging.info(stdout)
                ...
                if self.xianluType == 2:
                    status,stdout,stderr = self.dnsSsh.cmd("/app/opbin/dns/dnsapi -g %s -a up -d %s -l 2 -i %s"%(self.dns_game,server_url_tag,self.liantongIp))
                    logging.info("/app/opbin/dns/dnsapi -g %s -a up -d %s -l 2 -i %s"%(self.dns_game,server_url_tag,self.liantongIp))
                    logging.info(stdout)
                    if stdout.find("Record modify success") >= 0:
            ...
            if self.lianyun_add_dns:
                if self.add_dns:
                    ...
                    logging.info("服务器恢复需要通知联运重新做A记录%s A %s" %(server_url,self.dianxinIp))
                if self.xianluType == 2:
                    '''如果是CNAME记录，则电信跟联通只需要解析一条，否则需要解析每一条'''
                    if not self.add_dns:
                        logging.info("联通域名解析","%s A %s"%(server_url,self.liantongIp))
        except Exception,e1:
    ...
    def recoverServers(self):
        ...
        for serverflag in MainServers:
            ...
            for mix_serverflag in MixServers:
                mainserver=mix_serverflag['MainName']
                if servername != mainserver:
                    continue
                servermixname=mix_serverflag['servername']
                yx,quhao = servermixname.split("_")
                restart='yes'
                servermixdict = self.getUrl(yx,quhao)
                if  self.dianxinIp == self.liantongIp:
                    dns_ip_name = self.dianxinIp
                else:
                    dns_ip_name = servermixdict['server_url']
                self.cmd("test -d /app/%s_%s"%(self.game,mainserver),"游戏主服目录不存在")
                self.cmd("test ! -d /app/%s_%s/www_%s"%(self.game,mainserver,servermixname),"www_%s已经存在"%servermixname)
                try:
                    self.ssh.put("%s/../shell/%s"%(self.curdir,self.mix_deploy_script),remote_path=self.clientrootdir+"/shell")
                    self.ssh.put("%s/../shell/%s"%(self.curdir,self.deployCommonScript),remote_path=self.clientrootdir+"/shell")
                except Exception,e1:
                    self.statusCheck(False,str(e1)+"\nscp %s %s 到服务器失败"%(self.mix_deploy_script,self.deployCommonScript))
                mixserver_parameter = " '%s' '%s' '%s'  '%s'  '%s' '%s_%s' '%s' '%s' '%s' '%s' '%s' '%s' '%s'"%(self.game,self.language,servermixdict['title'],servermixname,servermixdict['server_url'],self.game,mainserver,dns_ip_name,self.www_ip,self.www_header,self.www_port,restart,self.mix_deploy_script,self.deploy_special_script)
                print mixserver_parameter
                recover_mix_stdout = self.cmd("source /etc/profile && sh %s/shell/%s %s"%(self.clientrootdir,self.deployCommonScript,mixserver_parameter), msg="恢复混服服务器失败!")
                logging.info(recover_mix_stdout)
                self.modifyBackstage(yx,quhao)
                self.dnsJiexi(servermixdict['server_url_tag'])
    def run(self):
        self.init()
        ###self.getUrl()
        self.getWip()

...
# 2015-05-20 Xiaoyu Modify AddContent func add dos2unix the file

import sys,os,json,commands,re,time,hashlib,urllib2
import optparse
...
def argInit():
    ...
    parser.add_option("-f","--frontName",dest="frontName",help="if change front path,give the front name")
    parser.add_option("-v","--executeVersionList",dest="executeVersionList",help="need update version list")
    parser.add_option("-z","--executeDbVersionList",dest="executeDbVersionList",help="need update db version list")
    ...
    if not null(options.sqlFile) or not null(options.replaceFile) or not null(options.addFile) or not null(options.addContent) or not null(options.specialScript):
...
def md5Check(filename):
    ...
    if filemd5.strip() != localmd5:
        ...
        sys.exit(1)
def splitCharacter(con,character=None):
    if character == None:
        paramList = con
        for c in ["|",",","\n",";"]:
            paramList = splitCharacter(paramList,c)
        return paramList
    if isinstance(con,list):
        l = []
        for item in con:
            l += splitCharacter(item,character)
        return l
    else:
        if not null(con):
            return con.split(character)
        else:
            return []
def download(file):
...
def getListFile(con):
    l = []
    for line1 in splitCharacter(con):
        if line1.find("="):
            s = line1.split("=",1)
...
def getFile():
    if haveFileDownload:
        download("md5.txt")
        for sql in splitCharacter(options.sqlFile):
            download(sql.strip())
        #if not null(options.sqlFile):
...
def getVersion():
    ...
    try:
        #request = urllib2.Request("%s://127.0.0.1/root/version.action"%httptype,headers={"host":host})
        #version = urllib2.urlopen(request).read().strip()
        ####使用系统命令，海外一些机器未装https模块导致urlopen调用失败
        cmdstr = 'curl -s -k -H "host:%s" %s://127.0.0.1/root/version.action' %(host,httptype)
        #print cmdstr
        version = os.popen(cmdstr).read().strip()
        versionJson = json.loads(version)
        versionTagList = options.versionTag.split(":")
        for i in versionTagList:
            if re.search(r'%s'%i,version):
                versionJson = versionJson[i]
            else:
                return None
        return versionJson
    except Exception,e1:
        print "WARNNING: get version warnning!msg:%s"%str(e1)
        return None
def versionCheck(executeVersionList,executeDbVersionList):
    mainVersionCheck = False
    dbVersionCheck = False
    if not null(executeVersionList):
        ...
        if curVersion == None :
            ...
            for v in s:
                if v.strip() == curVersion:
                    mainVersionCheck = True
                    break
    else:
        mainVersionCheck = True
    if not null(executeDbVersionList):
        dbversion = os.popen("pandora %s_%s -e 'select db_version from db_version'|grep -v db_version"%(options.game,options.server)).read().strip()
        for version in executeDbVersionList.split(","):
            if version.strip() == dbversion:
                dbVersionCheck = True
                break
    else:
        dbVersionCheck = True
    if mainVersionCheck and dbVersionCheck:
        return True
    else:
        return False
def backendChangeCheck():
    global oldBackendName,backendPathChange
    backendPathChange = False
    oldBackendName = None
    if not null(options.backendName):
        ...
        print "Old backend name is :" + oldBackendName
        if not re.match(r'^%s_[^/]*$'%options.game,oldBackendName):
            ...
            sys.exit(1)
        if options.backendName.strip() != oldBackendName:
            print "need change backend jar path"
            backendPathChange = True
def frontChangeCheck():
    global oldFrontName
    oldFrontName = None
    if not null(options.frontName):
        if not re.match(r'^%s_[^/]*$'%options.game,options.frontName):
            print "ERROR: new front name [%s] not match %s_.* front name"%(options.frontName,options.game)
            sys.exit(1)
        #oldFrontName = cmd("grep -E 'cdn[0-9]*.aoshitang.com.*version.js' /app/%s_%s/www*/Main.html|head -1|sed -e 's/.*aoshitang.com\\/\\(%s_[^\\/]*\\)\\/.*/\\1/'"%(options.game,options.server,options.game))
        oldFrontName = cmd("grep -E 'loadVersion.*http' /app/%s_%s/www*/Main.html|head -1|awk -F':' '{print $NF}'|cut -d'/' -f4"%(options.game,options.server))
        print "Old front version is :" + oldFrontName
        if not re.match(r'^%s_[^/]*$'%options.game,oldFrontName):
            print "ERROR: old front name [%s] not match %s_.* front name"%(oldFrontName,options.game)
            sys.exit(1)
def stopProcess():
...
def sqlUpdate():
    ...
    for sql in splitCharacter(options.sqlFile):
        cmd("dos2unix " + sql)
        print "start update sql file %s..."%sql
        cmd("pandora --update %s_%s < %s" %(options.game,options.server,sql))
        print "import sql file %s succ"%sql
...
def backendUpdate():
    ...
    sys.stdout.flush()
    appfile = "/app/%s_%s/backend/apps/app.properties"%(options.game,options.server)
    pluginfile = "/app/%s_%s/backend/apps/plugins.xml"%(options.game,options.server)
    cmd("cp %s %s/"%(appfile,backdir))
    updatejar = "app.properties"
    if os.path.exists(pluginfile):
        cmd("cp %s %s/"%(pluginfile,backdir))
        updatejar += " plugins.xml"
    print "start update jar load file %s..."%updatejar
    sys.stdout.flush()
    cmd("sed -i 's/%s/%s/g' %s"%(oldBackendName,options.backendName,appfile))
    if os.path.exists(pluginfile):
        cmd("sed -i 's/%s/%s/g' %s"%(oldBackendName,options.backendName,pluginfile))
    print "%s update finished!"%updatejar
    sys.stdout.flush()
def frontUpdate():
    wwwDirs = cmd("ls -d /app/%s_%s/www*"%(options.game,options.server)).split("\n")
    print wwwDirs
    #oldFrontName = cmd("grep -E 'cdn[0-9]*.aoshitang.com.*version.js' /app/%s_%s/www*/Main.html|head -1|sed -e 's/.*aoshitang.com\\/\\(%s_[^\\/]*\\)\\/.*/\\1/'"%(options.game,options.server,options.game))
    oldFrontName = cmd("grep -E 'loadVersion.*http' /app/%s_%s/www*/Main.html|head -1|awk -F':' '{print $NF}'|cut -d'/' -f4"%(options.game,options.server))
    print "oldFrontName:",oldFrontName
    for www in wwwDirs:
        baseDir = os.path.basename(www)
        wwwBackDir = "%s/%s"%(backdir,baseDir)
        cmd("mkdir -p %s"%wwwBackDir)
        print "start backup %s/Main.html ...."%www
        cmd("cp %s/Main*.html %s/"%(www,wwwBackDir))
        print "start update %s/Main.html ..."%www
        cmd("sed -i 's/%s/%s/g' %s/Main*.html"%(oldFrontName,options.frontName,www))
    sys.stdout.flush()
def replcaceFile():
...
def addContent():
    for i in getListFile(options.addContent):
        ...
        cmd("dos2unix /app/%s_%s/%s"%(options.game,options.server,oldfile))
if __name__ == "__main__":
    ...
    frontChangeCheck()
    #create work dir
    #if haveFileDownload or not null(options.backendName):
    backdir = "updateBackupDir"
    if haveFileDownload or backendPathChange:
        #nullCheck(options.executeVersionList,"update version list must be given")
        if null(options.executeVersionList) and null(options.executeDbVersionList):
            print "ERROR: the version or db version nedd update must be given!"
            sys.exit(1)
        #check the version need update or not
        if not versionCheck(options.executeVersionList,options.executeDbVersionList):

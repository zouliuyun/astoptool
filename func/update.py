...
def executeUpdate(sshObj,ip,server,sqlFile,backendName,executeVersionList,executeDbVersionList,restart,www_header,www_ip,www_port,replaceFile,addFile,addContent,specialScriptExecuteFirst,versionTag,specialScriptFileName,remoteResourceDir,frontName):
    '''
    执行更新脚本
    '''
    print "开始更新:" + server
    sys.stdout.flush()
    argStr = "nohup python " + remoteShellPath + "/" + updateScript + " -g " + state.options.game + " -s " + server + " -H " + www_header + " -i " + www_ip + " -p " + www_port + " -D " + remoteWorkDir 
    if versionTag:
        argStr += " -V " + versionTag
    ...
    if executeDbVersionList:
        argStr += " -z '" + executeDbVersionList + "'"
    if restart:
        argStr += " -R '" + restart + "'"
    if replaceFile:
...
def resourceFileCheck(resourceDir,file):
    if not os.path.exists("/app/online/%s/update/%s/%s"%(state.game,resourceDir,file)):
        raise Exception("/app/online/%s/update/%s/%s 不存在ftp中，请确认!"%(state.game,resourceDir,file))
...
def splitCharacter(con,character=None):
    ...
    if isinstance(con,list):
        ...
        if not null(con):
            ...
            return []
def sqlFileCheck(resourceDir,con):
    for sql in splitCharacter(con):
        resourceFileCheck(resourceDir,sql.strip())
def getListFile(con):
    l = []

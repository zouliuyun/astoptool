...
class deploy:
    def __init__(self,game,language,servername,mainserver,restart,title,gameurl,asturl,skipcheck,starttime=None):
        '''游戏混服部署任务
    ...
    def init(self):
        ...
        if len(self.mail_title) > 50:
            self.mail_title = self.mail_title[0:47] + "..."
    ...
    def addBackstage(self,yx,quhao):
        ...
        try:
            ...
            addServer = backstage.addBackstage(self.backstage_interface_url,data,header)
            logging.info(yx + "_" + quhao + "后台接口调用结果:" + addServer["response"])
            if addServer["status"]:
                errorAdd = True
                #设置混服
                data1 = {}
                ...
                mixServer = backstage.mixBackstage(self.backstage_interface_url,data1,header)
                logging.info("后台混服设置结果:" + str(mixServer["response"]))
                if mixServer["status"]:
                    errorAddMix = True
        except Exception,e1:
            print "add backstage %s_%s err:%s"%(yx,quhao,str(e1))
        return errorAdd,errorAddMix
    def bigMixDeploy(self):
        self.html.add("混服数目",str(len(self.sList)))
        ...
        for i in self.sList:
            ...
            try:
                ...
                os.system("chown -R astd.astd %s"%tempDir)
        os.system("chown -R astd.astd %s"%tempDir)
        ...
        try:
            ...
            if self.special_script.strip() != "":
                self.ssh.put(self.curdir + "/../shell/"+self.special_script,remote_path="/app/%s_%s/"%(self.game,self.mainserver))
        except Exception,e1:
            self.statusCheck(False,"上传zip包或者布服脚本失败!"+str(e1))
        os.system("rm -rf %s"%zipfile)
        os.system("rm -rf %s.zip"%zipfile)
        #mixResult = self.cmd("cd /app/%s_%s && sh %s -m '%s_%s' -l '%s' -r '%s' -s '%s'"%(self.game,self.mainserver,self.mix_deploy_script,self.game,self.mainserver,self.servername,self.restart,self.special_script))
        mixResult = self.cmd("source /etc/profile && sh /app/%s_%s/%s -m '%s_%s' -l '%s' -r '%s' -s '%s'"%(self.game,self.mainserver,self.mix_deploy_script,self.game,self.mainserver,self.servername,self.restart,self.special_script))
        logging.info(mixResult)
        if mixResult.find("backstage添加失败") >= 0:
            self.html.add("backstage添加","失败",color="red")
            self.htmlStatus = False
        if mixResult.find("error:special change failed") >= 0:
            self.html.add("特殊配置修改","失败",color="red")
            self.htmlStatus = False
        self.ssh.cmd("rm -f /app/%s_%s/%s"%(self.game,self.mainserver,self.mix_deploy_script))
        if self.special_script.strip() != "":
            self.ssh.cmd("rm -f /app/%s_%s/%s"%(self.game,self.mainserver,self.special_script))

        #本地特殊脚本，临时给天将手游用
        if self.special_script_local.strip() != "":
            try:
                logging.info('开始执行特殊脚本local...')
                from subprocess import Popen, PIPE, STDOUT
                sub_process = Popen("{curdir}/../shell/{script_name} -g {game} -m {mainserver} -s {servername} -i {external_ip}".format(curdir=self.curdir, script_name=self.special_script_local, game=self.game, mainserver=self.mainserver, servername=self.servername, external_ip=self.dianxinIp), shell=True, stdin=PIPE, stdout=PIPE, stderr=STDOUT) 
                result_sub, _ = sub_process.communicate()
                result_rc = sub_process.returncode
                logging.info(result_sub)
                if result_rc != 0:
                    self.statusCheck(False,"特殊脚本local执行失败!" + result_sub)
            except Exception,e1:
                self.statusCheck(False,"特殊脚本local执行失败!"+str(e1))
    ...
    def webMixDeploy(self):
        ...
        try:
            ...
            if self.special_script.strip() != "":
                self.ssh.put(self.curdir + "/../shell/"+self.special_script,remote_path="/app/%s_%s/"%(self.game,self.mainserver))
            self.ssh.put(self.curdir + "/../shell/"+deployCommonScript,remote_path="/app/%s_%s/"%(self.game,self.mainserver))
        except Exception,e1:
            logging.error("复制文件到服务器失败！err:\n" + str(e1))
            self.statusCheck(False,"复制文件失败！\n"+ str(e1))
        #mixResult = self.cmd("cd /app/%s_%s && sh %s '%s' '%s' '%s' '%s' '%s' '%s_%s' '%s' '%s' '%s' '%s' '%s' '%s' '%s'"%(self.game,self.mainserver,deployCommonScript,self.game,self.language,self.title,self.servername,self.server_url,self.game,self.mainserver,self.dns_ip_name,self.www_ip,self.www_header,self.www_port,self.restart,self.mix_deploy_script,self.special_script))
        mixResult = self.cmd("source /etc/profile && sh /app/%s_%s/%s '%s' '%s' '%s' '%s' '%s' '%s_%s' '%s' '%s' '%s' '%s' '%s' '%s' '%s'"%(self.game,self.mainserver,deployCommonScript,self.game,self.language,self.title,self.servername,self.server_url,self.game,self.mainserver,self.dns_ip_name,self.www_ip,self.www_header,self.www_port,self.restart,self.mix_deploy_script,self.special_script))
        logging.info(mixResult)
        if mixResult.find("ERROR: nginx error") >= 0:
            self.html.add("nginx","nginx配置文件错误或者重启失败",color="red")
            self.htmlStatus = False
        self.ssh.cmd("rm -f /app/%s_%s/%s"%(self.game,self.mainserver,self.mix_deploy_script))
        self.ssh.cmd("rm -f /app/%s_%s/%s"%(self.game,self.mainserver,deployCommonScript))
        if self.special_script.strip() != "":
            self.ssh.cmd("rm -f /app/%s_%s/%s"%(self.game,self.mainserver,self.special_script))
    def run(self):
        ...
        for everyServer in self.sList:
            ...
            if self.have_backstage_interface:
                ...
                if not addBGStatus:
                    addBGError.append(yx+"_"+quhao)
                    self.htmlStatus = False
                if not addBGMixStatus:
                    addBGMixError.append(yx+"_"+quhao)
                    self.htmlStatus = False
        if len(addBGError) > 0 :
            self.html.add("添加后台失败游戏服",",".join(addBGError),color="red")
        else:
            self.html.add("添加后台","成功",color="green")
        if len(addBGMixError) > 0 :
            self.html.add("设置后台混服失败游戏服",",".join(addBGMixError),color="red")
        else:

...
def add_serverlist(parser):
    ...
    parser.add_argument("-f", "--serverfile", dest="serverfile", default=None, help="服务器列表文件(列表文件为绝对路径)")
...
def get_func(options):
    from func import get
    get.get(options.localfile, options.remotefile)
...
def deploymix_func(options):
    ...
    newserver = deployMix.deploy(game, language, options.servername, options.mainserver, options.restart, title=options.title, gameurl=options.gameurl, asturl=options.asturl, skipcheck=options.skipcheck, starttime=options.starttime)
    newserver.run()
...
def mobileWwwTestUpdate_func(options):
    from func import mobile_www_test_update1
    mobile_www_test_update1.init(game, language, options.version, options.updateType, options.hd)
def mobileWwwUpdate_func(options):
    from func import mobile_www_update1
    mobile_www_update1.update(game, language, options.version, options.updateType, options.hd)
def update_func(options):
    ...
    update.update(options.sqlFile, options.backendName, options.executeVersionList, options.executeDbVersionList, options.restart, options.resourceDir, options.replaceFile, options.addFile, options.addContent, options.specialScript, options.executeFirst, options.frontName, options.updateTemplate)
...
def addPartner_fun(options):
    from func import addPartner
    addPartner = addPartner.addPartner(game,language,options.oldyx,options.newyx,options.official_url,options.addiction_url,options.bbs_url,options.cname,options.pay_url,options.allow_ip)
    addPartner.run()
...
def arg_init():
    ...
    #新联运添加
    addPartner_parser = subparser.add_parser("addPartner", help="页游新联运添加")
    add_main(addPartner_parser)
    addPartner_parser.add_argument("-n", "--newyx", dest="newyx", required=True, help="新增联运标示符,例如yy")
    addPartner_parser.add_argument("-c", "--cname", dest="cname", required=True, help="新增联运中文名,例如多玩")
    addPartner_parser.add_argument("-O", "--official_url", dest="official_url", required=True, help="官网url")
    addPartner_parser.add_argument("-A", "--addiction_url", dest="addiction_url", required=True, help="防沉迷url")
    addPartner_parser.add_argument("-B", "--bbs_url", dest="bbs_url", required=True, help="论坛url")
    addPartner_parser.add_argument("-P", "--pay_url", dest="pay_url", required=True, help="充值url")
    addPartner_parser.add_argument("-a", "--allow_ip", dest="allow_ip", required=True, help="白名单ip")
    addPartner_parser.add_argument("-o", "--oldyx", dest="oldyx", required=True, help="指定生成模板的参照联运，比如:49you")
    addPartner_parser.set_defaults(func=addPartner_fun)   
    #命令执行参数列表
    ...
    #下载文件参数列表
    get_parser = subparser.add_parser("get", help="文件下载")
    add_main(get_parser)
    get_parser.add_argument("-F", "--localfile", dest="localfile", required=True, help="本地文件")
    get_parser.add_argument("-R", "--remotefile", dest="remotefile", required=True, help="远端文件路径")
    add_serverlist(get_parser)
    ...
    deploymix_parser.add_argument("-r", "--restart", dest="restart", choices=["yes", "no"], default="no", help="部署游戏混服是否重启游戏服")
    ...
    deploymix_parser.add_argument("-S", "--starttime", dest="starttime", help="游戏开服时间，如果手游新加联运到已开游戏服可以设置为当前时间")
    ...
    mobileWwwTestUpdate_parser.add_argument("-t", "--updateType", dest="updateType", required=True, help="更新类型，比如:appstore")
    ...
    mobileWwwUpdate_parser.add_argument("-t", "--updateType", dest="updateType", required=True, help="更新类型，比如:appstore")
    ...
    update_parser.add_argument("--updateTemplate", dest="updateTemplate", default="null", help="指定需要更新的模板类型")
    ...
    #测试环境后端动更 
    from func.testEnvHotswap import add_args_for_testEnvHotswap
    def testEnvHotswap_func(options):
        from func import testEnvHotswap
        release = testEnvHotswap.Release(options)
        release.hotswap()

    testEnvHotswap_parser = subparser.add_parser("testEnvHotswap", help="测试环境后端动更")
    add_args_for_testEnvHotswap(testEnvHotswap_parser)
    testEnvHotswap_parser.set_defaults(func=testEnvHotswap_func)

    #测试环境上传/下载配置文件
    from func.test_env_load_file import add_testEnvLoadFile_parser
    add_testEnvLoadFile_parser(subparser)

    #测试环境数据库导出
    from func.test_env_export_db import add_testEnvExportDB_parser
    add_testEnvExportDB_parser(subparser)

    #手游前端测试环境配置文件修改
    from func.test_env_modify_mobile_www_config import add_testEnvModifyMobileWwwConfig_parser
    add_testEnvModifyMobileWwwConfig_parser(subparser)

    #部署新的match（必须要已经有至少一个正在运行的match作为模板)
    from func.deployMatch import add_deployMatch_parser
    add_deployMatch_parser(subparser)

    #刷新内存数据库，比如reload sdata, clear cache, 执行sql
    from func.flush_mem_data import add_flushMemData_parser
    add_flushMemData_parser(subparser)
    
    #手游前端服务器清理过期的资源以释放磁盘空间
    from func.mobile_www_clean_up import add_mobileWwwCleanUp_parser
    add_mobileWwwCleanUp_parser(subparser)

    #新的手游前端更新模块
    from func.mobile_www_release import add_mobileWwwRelease_parser
    add_mobileWwwRelease_parser(subparser)

    #手游前端强制使测试环境与正式环境同步一次，将会删除测试环境中多余的目录和文件
    from func.test_env_mobile_www_sync import add_testEnvMobileWwwSync_parser
    add_testEnvMobileWwwSync_parser(subparser)
 
    #合服
#    from func.merge import add_merge_parser
#    add_merge_parser(subparser)

    from func.web_www_incremental_update import add_webWwwIncrementalUpdate_parser
    add_webWwwIncrementalUpdate_parser(subparser)
    #充值中控重启更新
    from func.payProxy_Update import payProxy_Update
    payProxy_Update(subparser)

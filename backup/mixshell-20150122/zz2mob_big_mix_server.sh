...
game_url=$(grep -E "^[a-z0-9]*\.?game.url" /app/$Main_server/backend/apps/${Main_server_yx}.properties|cut -d'=' -f2|xargs echo|cut -d'/' -f3)
...
do
	...
	sed -i "s/\(^[a-z0-9]*\.\?game.url = \).*/\1http:\/\/${game_url}/g" ${mix_server_yx}.properties
	sed -i "s/\(^[a-z0-9]*\.\?serverid = \).*/\1${mix_server_quhao}/g" ${mix_server_yx}.properties
	sed -i "s/\(^[a-z0-9]*\.\?serverids = \).*/\1S${mix_server_quhao}/g" ${mix_server_yx}.properties
	...
	#修改运营商标示
	sed -i "s/\(^[a-z0-9]*\.\?yx = .*\)/\1,${mix_server_yx}/g" /app/${Main_server}/backend/apps/server.properties

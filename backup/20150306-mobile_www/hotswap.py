...
def execute(sshObj,ip,server,type,keyword,www_ip,www_port,www_header):
    ...
    if type == "update":
        ...
        time.sleep(2)
        status,stdout,err = sshObj.cmd('grep -E -A 20 "reload.*%s" %s | grep "reload succ"'%(keyword,logfile))
    else:
        updateLog = cmd(sshObj,server,"cd %s/hotswap && ./remote %s" %(workdir,pid.strip()))
        time.sleep(2)
        status,stdout,err = sshObj.cmd('grep "%s" %s'%(keyword,logfile))
    if status != 0:
        sys.stdout.write( "[%s]\n%s 获取关键字失败:\n%s\n"%(server,updateLog,err) )

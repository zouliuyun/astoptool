...
class deploy:
        ...
        try:
            if self.add_dns:
                ...
                if self.xianluType == 2:
                    ...
                    logging.info("/app/opbin/dns/dnsapi -g %s -a add -d %s -l 2 -i %s"%(self.game,self.server_url_tag,self.liantongIp))
                    logging.info(stdout)
                    if stdout.find("Record add success") >= 0:
    ...
    def addBackstage(self):
        try:
            status,stdout = commands.getstatusoutput("curl -s --retry 3 --retry-delay 5 -H 'host:%s' --data 'name=S%s&server_flag=%s&n_ip=%s&w_ip=%s&web_url=%s&cnc_ip=%s&startTime=%s' 'http://%s/backStage!addServer.action'"%(self.backstage_header,self.quhao,self.yx,self.ip,self.dianxinIp,self.web_url,self.liantongIp,self.starttime,self.backstage_interface_url))
            logging.info("后台接口调用结果:" + str(stdout))
            if status == 0:
                ...
                if statusCode == 1:
                    ...
                    self.html.add("后台添加",jsonResult["msg"],color="red")
                    self.htmlStatus = False
        except Exception,e1:
            self.html.add("后台添加",str(e1)+"\n添加后台失败",color="red")
            logging.error("后台添加失败")
            self.htmlStatus = False
    ...
    def run(self):
        ...
        newserver_parameter = " '%s' '%s' '%s'  '%s'  '%s' '%s' '%s' '%s' '%s' '%s' '%s' '%s' '%s' '%s' '%s' '%s'"%(self.game,self.language,self.servername,self.title,self.server_url,self.dns_ip_name,self.is_mobile,self.cleartime,self.innit_sql,self.www_dir_type,self.www_ip,self.www_port,self.www_header,self.clear_script,self.template_tar_dir,self.template_exclude_dir)
        deploy_stdout = self.cmd("source /etc/profile && sh %s/shell/%s %s"%(self.clientrootdir,self.deploy_script,newserver_parameter), msg="部署服务器失败!")
        ...
        tcpport = self.cmd("grep port /app/%s_%s/backend/apps/conf.xml|cut -d'>' -f2|cut -d'<' -f1"%(self.game,self.servername))

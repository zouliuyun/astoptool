    ...
    def dnsJiexi(self):
        try:
            if self.add_dns:
                ...
                status,stdout,stderr = dnsSsh.cmd("/app/opbin/dns/dnsapi -g %s -a up -d %s -l 1 -i %s"%(self.game,self.server_url_tag,self.dianxinIp))
                logging.info("/app/opbin/dns/dnsapi -g %s -a up -d %s -l 1 -i %s"%(self.game,self.server_url_tag,self.dianxinIp))
                logging.info(stdout)

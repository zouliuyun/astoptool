    ...
    def dnsJiexi(self,server_url_tag):
        try:
            if self.add_dns:
                ...
                status,stdout,stderr = self.dnsSsh.cmd("/app/opbin/dns/dnsapi -g %s -a up -d %s -l 1 -i %s"%(self.game,server_url_tag,self.dianxinIp))
                ...
                if self.xianluType == 2:
                    status,stdout,stderr = self.dnsSsh.cmd("/app/opbin/dns/dnsapi -g %s -a up -d %s -l 2 -i %s"%(self.game,server_url_tag,self.liantongIp))
                    logging.info("/app/opbin/dns/dnsapi -g %s -a up -d %s -l 2 -i %s"%(self.game,server_url_tag,self.liantongIp))
                    logging.info(stdout)
                    if stdout.find("Record modify success") >= 0:

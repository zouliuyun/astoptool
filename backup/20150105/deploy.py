...
import commands
import check,common,ssh,mailhtml,sendmail,serverlist,backstage
        ...
        if self.big_mix_server:
            ...
            try:
                ...
                if not big_mix_list or big_mix_list.strip() == "":
                    ...
                    import deployMix
                    try:
                        mixserverlist = ",".join([ "%s_%s"%(i.strip(),self.quhao) for i in big_mix_list.split(",") ])
                        mixserver = deployMix.deploy(self.game,self.language,mixserverlist,self.servername,"yes",None,None,None,False)
                        mixserver.run()

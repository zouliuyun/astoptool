...
class SCPClient(object):
    ...
    def _send_files(self, files):
        for name in files:
            ...
            starttime = time.time()
            while file_pos < size:
                #channel senddata retry
                errmsg = None
                sendstr = file_hdl.read(buff_size)
                for i in range(5):
                    try:
                        chan.sendall(sendstr)
                        break
                    except SocketTimeout,e1:
                        print "WARNNING:channel send data timeout,msg:%s,retry...\n"%str(e1)
                        errmsg = e1
                        time.sleep(1)
                else:
                    raise errmsg
    ...
    def _channel_recv(self,channel,attr,buffer):
        method = getattr(channel,attr)
        for i in range(5):
            try:
                #msg = channel.recv(buffer)
                msg = method(buffer)
                return msg
                break
            except SocketTimeout,e1:
                print "WARNNING:recive time out,retry...\n"
                time.sleep(1)
        else:
    ...
    def _recv_confirm(self):
        ...
        #except SocketTimeout,e1:
        #    #raise SCPException('Timout waiting for scp response')
        msg = self._channel_recv(self.channel,"recv",512)
        ...
        if msg and msg[0:1] == b'\x00':
            ...
            msg = self._channel_recv(self.channel,"recv_stderr",512)
    ...
    def _recv_all(self):
        ...
        while not self.channel.closed:
            ...
            msg = self._channel_recv(self.channel,"recv",1024)
    ...
    def _recv_file(self, cmd):
        ...
        try:
            ...
            while pos < size:
                ...
                getstr = self._channel_recv(chan,"recv",buff_size)
                file_hdl.write(getstr)
                pos = file_hdl.tell()
                if self._progress:
                    nowtime = time.time()
                    if nowtime - starttime > 5:
            ...
            #msg = chan.recv(512)
            msg = self._channel_recv(chan,"recv",512)
            if msg and msg[0:1] != b'\x00':
